'use strict';
module.exports = function createRateLimitChecker(EDGECONNECTOR_RATELIMIT) {
  // Configure rate limit. The following format is accepted for EDGECONNECTOR_RATELIMIT:
  ...
  var rateLimitConfig = /^(\d+) (\d+)(?:\s*$|\s+(.+)$)/.exec(EDGECONNECTOR_RATELIMIT);
  if (!rateLimitConfig) {
    ...
    unlimitedPattern.trim().split(/\s+/).forEach(function(unlimitedHost, i) {
      ...
      if (startsWithSlash || endsWithSlash) {
        if (unlimitedHost.length === 1 || !startsWithSlash || !endsWithSlash) {
          throw new Error('Invalid EDGECONNECTOR_RATELIMIT. Regex at index ' + i +
  ...
  var rateLimitMessage = 'The number of requests is limited to ' + maxRequestsPerPeriod +
    ...
    'Please self-host EDGE Connector if you need more quota. ' +
    'See https://github.com/Rob--W/edge-connector#demo-server';
